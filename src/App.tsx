...
// --- STRICT TYPES TO FIX ALL TS ERRORS ---
interface BuildType {
  name: string;
  price: number;
  material: string;
...
export default function App() {
  ...
  const [selectedDrivetrainFilters, setSelectedDrivetrainFilters] = useState<string[]>([]); // NEW
  ...
  const [isDrivetrainOpen, setIsDrivetrainOpen] = useState(false); // NEW
...
const filteredBikes = useMemo(() => {
    return BIKES.filter(bike => {
      // Keep garage logic intact
      if (showGarage) return bike.builds.some(build => favorites.includes(build.id));
      
      // 1. Model-Level Filters (Brand & Travel apply to the whole bike)
      ...
      // 2. Build-Level Filters (We must find AT LEAST ONE build that perfectly matches ALL of these simultaneously)
      ...
      // The bike only shows up if the brand/travel match AND it has a build that fits the price/specs
  ...
  const clearFilters = () => {
    ...
    setSelectedDrivetrainFilters([]); // NEW
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100 selection:text-blue-900 flex flex-col relative">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 sm:h-20 flex items-center justify-between">          <div 
              ...
              <div className="w-full pb-2 sm:pb-4 mt-4 sm:mt-8">
                <div className="relative w-full bg-[#0B1121] rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-800 p-8 sm:p-12 lg:p-16 flex flex-col items-center justify-center text-center">
                  ...
                  <div className="relative z-10 max-w-2xl">
                    ...
                    {/* Only show the compare button if they have at least 2 bikes */}
                    {favorites.length > 1 && (
                       <button 
                         onClick={() => {
                         ...
                         className="mt-8 bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-8 rounded-xl transition-colors shadow-lg shadow-blue-900/50 inline-flex items-center gap-2"
                       >
                         <Scale size={18} />
                         Compare Garage
                       </button>
...
<div className="space-y-3 sm:space-y-4">
                            {/* --- EDITORIAL OVERLINE INSTEAD OF AI PILL BADGE --- */}
                            ...
                            {/* --- SOLID INDUSTRIAL TEXT INSTEAD OF GRADIENT --- */}
                            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-black uppercase tracking-tighter text-white leading-[1.05]">
                              FIND YOUR NEXT <br />
                          ...
                          <div className="flex flex-wrap gap-4 pt-2">
                             <button 
                               ...
                               className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 px-6 rounded-xl transition-colors shadow-lg shadow-blue-900/50 flex items-center justify-center gap-2 w-full sm:w-auto"
                             >
                               <Filter size={18} />
                               Start Filtering
                             </button>
                             <button 
                               onClick={() => setSelectingRig('A')}
                               className="bg-slate-800 hover:bg-slate-700 text-white font-bold py-3 px-6 rounded-xl transition-colors border border-slate-700 flex items-center justify-center gap-2 w-full sm:w-auto"
                             >
                               <Scale size={18} />
                               Head-to-Head
                             </button>
                        ...
                        <div className="md:w-1/2 w-full mt-8 md:mt-0 relative z-10 aspect-[4/3] sm:aspect-video md:aspect-[4/3] lg:aspect-[4/3] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-slate-700/50 transform md:-rotate-2 hover:rotate-0 transition-transform duration-500 overflow-hidden ring-1 ring-white/10">
                          {heroImages.map((src, index) => (
                            <img 
                              key={src} src={src} alt={`Dream Rig ${index + 1}`} 
                  ...
                  <div className="w-full mb-8 sm:mb-12 [mask-image:_linear-gradient(to_right,transparent_0,_black_10%,_black_90%,transparent_100%)]">
                    <div className="w-full bg-white border-y border-slate-200 py-3 overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                      <div className="flex items-center space-x-8 px-4 w-max animate-[scroll_30s_linear_infinite] hover:[animation-play-state:paused]">
                        {randomizedBrands.map(b => b.logo && (
                          <img key={`${b.brand}-1`} src={b.logo} alt={b.brand} className="h-6 md:h-8 max-w-[120px] md:max-w-[140px] w-auto object-contain grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-300" />
                        ))}
                        {randomizedBrands.map(b => b.logo && (
                          <img key={`${b.brand}-2`} src={b.logo} alt={b.brand} className="h-6 md:h-8 max-w-[120px] md:max-w-[140px] w-auto object-contain grayscale opacity-50 hover:grayscale-0 hover:opacity-100 transition-all duration-300" />
                        ))}
                      </div>
                    </div>
                  </div>

                  {/* --- NEW: TRENDING RIGS CAROUSEL --- */}
                  <div className="w-full mb-12 sm:mb-16">
                    <div className="flex items-center justify-between mb-6 px-2">
                      <h3 className="text-2xl font-extrabold text-slate-900 tracking-tight">Trending Rigs</h3>
                      <div className="text-xs font-bold text-slate-400 uppercase tracking-wider hidden sm:block">Swipe to explore &rarr;</div>
                    </div>
                    
                    <div className="flex overflow-x-auto gap-4 sm:gap-6 pb-6 px-2 snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                      {['Vala', 'Levo', 'Altitude', 'Bullit', 'Moterra']
                        ...
                        <div 
                          ...
                          className="min-w-[85vw] sm:min-w-[320px] md:min-w-[360px] snap-center bg-white rounded-3xl p-6 shadow-sm border border-slate-200 hover:border-blue-500 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer group flex flex-col"
                        >
                          <div className="w-full h-48 bg-[#F3F3F3] rounded-2xl mb-6 relative overflow-hidden flex items-center justify-center">
                            <img src={bike.image} alt={bike.model} className="absolute inset-0 w-full h-full object-contain scale-110 group-hover:scale-125 transition-transform duration-500" crossOrigin="anonymous" />
                          </div>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
                      ...
                      >
                        ...
                        Filter Rigs {totalActiveFilters > 0 && <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full text-xs">{totalActiveFilters}</span>}                      
                      </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 mt-4 sm:mt-8">
                {ALL_BUILDS.filter(b => favorites.includes(b.id)).map(build => (
                  ...
                  >
                    {/* Quick Remove Button */}
                    <button 
                      onClick={(e) => { e.stopPropagation(); toggleFavorite(build.id); }}
                      ...
                      {group.bikes.map(bike => (
                        ...
                        >
                          <div className="w-full p-0 h-100 overflow-hidden relative shrink-0">
                            <img src={bike.image} alt={bike.model} className="w-full h-full object-contain scale-110" crossOrigin="anonymous" />
                          </div>
                          <div className="p-6 flex-1 flex flex-col justify-center min-w-0">
                            <div className="flex justify-between items-center text-xs uppercase mb-1">
                              ...
                              <span className="text-gray-400 font-medium">{bike.suspension}</span>
                            </div>
                ...
                <div className="bg-white rounded-2xl overflow-hidden shadow-lg border border-slate-100 flex flex-col">
                  ...
                  <div className="p-8 text-center flex flex-col items-center">
                    {eMTBData.find(b => b.brand === selectedBike.brand)?.logo && (
                      <img src={eMTBData.find(b => b.brand === selectedBike.brand)?.logo} alt={selectedBike.brand} className="h-10 w-auto object-contain mx-auto mb-4" crossOrigin="anonymous" />
                    )}
                  ...
                  <div key={build.id} onClick={() => { setSelectedBuildId(build.id); setView('calculator'); }} className="group bg-white rounded-2xl p-5 sm:p-6 shadow-sm border border-slate-200 hover:shadow-md hover:border-blue-600 hover:scale-[1.01] transition-all duration-300 cursor-pointer flex flex-col gap-4 sm:gap-5">
                    <div className="flex items-start sm:items-center justify-between gap-4">
                      <div className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4">
                        ...
                        <span className="text-lg sm:text-xl font-extrabold text-slate-900 bg-slate-50 px-2 sm:px-3 py-0.5 sm:py-1 rounded-lg border border-slate-100 inline-block w-fit">{formatPrice(build.price)}</span>
                      </div>
...
{/* --- COMPARE VIEW --- */}
        {view === 'compare' && (
          <div className="animate-in fade-in slide-in-from-bottom-8 duration-500 pb-12">
            ...
            <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl border border-slate-200 overflow-hidden flex flex-col">
              
              {/* --- PREMIUM COMPARE HEADER --- */}
              <div className="bg-[#0B1121] p-8 sm:p-12 relative overflow-hidden flex items-center justify-between border-b border-slate-800">
                ...
                <div className="relative z-10">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <span className="text-slate-400 text-[10px] sm:text-xs font-black uppercase tracking-[0.2em]">Spec Check</span>
                  </div>
                  ...
                  <p className="text-slate-400 mt-4 font-medium max-w-xl text-sm sm:text-base">
                    Stack two rigs side-by-side to compare geometry, motors, and components before you calculate the damage.
                  </p>
                </div>
                
                <div className="hidden lg:flex items-center justify-center relative z-10 opacity-20">
                  <Scale size={120} className="text-blue-500" />
                </div>
              </div>

              {/* --- RIG A / RIG B DARK BLOCKS --- */}
              <div className="grid grid-cols-2 divide-x divide-slate-800 bg-[#0B1121] relative overflow-hidden">
                <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSIyMCI+PHBhdGggZD0iTTAgMGgyMHYyMEgwVjB6bTEgMWgxOHYxOEgxdjE4eiIgZmlsbD0icmdiYSgyNTUsIDI1NSwgMjU1LCAwLjAzKSIgZmlsbC1ydWxlPSJldmVub2RkIi8+PC9zdmc+')] opacity-50 pointer-events-none"></div>                
                <div className="flex flex-col">
                  <div className="p-3 sm:p-6 border-b border-slate-800">
                    <label className="block text-[10px] sm:text-xs font-bold text-blue-400 uppercase tracking-wider mb-2">Rig A</label>
                    <button onClick={() => { setSelectingRig('A'); setSelectorBrand(rigA ? String(rigA.brand) : brands[0]); }} className="w-full bg-slate-800 border border-slate-700 hover:border-blue-500 text-white text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl p-2 sm:p-3 text-left flex justify-between items-center transition-colors">
                  ...
                  {rigA && (
                    <>
                      <div className="w-full aspect-[4/3] sm:aspect-video bg-slate-800/50 relative overflow-hidden flex items-center justify-center">
                        <img src={rigA.image} alt={rigA.fullName} className="absolute inset-0 w-full h-full object-contain scale-110 sm:scale-125 drop-shadow-2xl" crossOrigin="anonymous" />
                      </div>
                      <div className="p-4 sm:p-8 text-center border-b border-slate-800 flex-1 flex flex-col justify-center">
                        <h3 className="text-sm sm:text-2xl font-extrabold text-white leading-tight mb-1 sm:mb-2">{rigA.model} <br className="sm:hidden" />{rigA.name}</h3>
                        <div className="text-sm sm:text-xl font-bold text-blue-400">{formatPrice(rigA.price)}</div>
                      </div>
                    </>
                  )}
                </div>
                <div className="flex flex-col">
                  <div className="p-3 sm:p-6 border-b border-slate-800">
                    <label className="block text-[10px] sm:text-xs font-bold text-emerald-400 uppercase tracking-wider mb-2">Rig B</label>
                    <button onClick={() => { setSelectingRig('B'); setSelectorBrand(rigB ? String(rigB.brand) : brands[0]); }} className="w-full bg-slate-800 border border-slate-700 hover:border-emerald-500 text-white text-xs sm:text-sm font-semibold rounded-lg sm:rounded-xl p-2 sm:p-3 text-left flex justify-between items-center transition-colors">
                  ...
                  {rigB && (
                    <>
                      <div className="w-full aspect-[4/3] sm:aspect-video bg-slate-800/50 relative overflow-hidden flex items-center justify-center">
                        <img src={rigB.image} alt={rigB.fullName} className="absolute inset-0 w-full h-full object-contain scale-110 sm:scale-125 drop-shadow-2xl" crossOrigin="anonymous" />
                      </div>
                      <div className="p-4 sm:p-8 text-center border-b border-slate-800 flex-1 flex flex-col justify-center">
                        <h3 className="text-sm sm:text-2xl font-extrabold text-white leading-tight mb-1 sm:mb-2">{rigB.model} <br className="sm:hidden" />{rigB.name}</h3>
                        <div className="text-sm sm:text-xl font-bold text-emerald-400">{formatPrice(rigB.price)}</div>
                      </div>
                    </>
                  )}
                </div>
      ...
      {isFilterModalOpen && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-4" style={{ zIndex: 9998 }}>
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md" onClick={() => setIsFilterModalOpen(false)} />
          <div className="bg-white rounded-xl shadow-[0_0_100px_rgba(0,0,0,0.5)] w-[90vw] max-w-md max-h-[80vh] flex flex-col relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-4 sm:p-6 overflow-y-auto space-y-4 flex-1">
              <div>
                <button onClick={() => setIsBrandOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isBrandOpen}>
              ...
              <div>
                <button onClick={() => setIsMotorOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isMotorOpen}>
              ...
              <div>
                <button onClick={() => setIsTorqueOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isTorqueOpen}>
              ...
              {/* --- NEW DRIVETRAIN BLOCK --- */}
              <div>
                <button onClick={() => setIsDrivetrainOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isDrivetrainOpen}>
              ...
              <div>
                <button onClick={() => setIsWheelsOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isWheelsOpen}>
              ...
              <div>
                <button onClick={() => setIsTravelOpen(v => !v)} className="w-full flex items-center justify-between gap-4 p-3 bg-slate-50 rounded-md" aria-expanded={isTravelOpen}>
                    ...
                    {travels.map(tvl => {
                      ...
                      return (
                        <label key={tvl} className="flex items-center gap-3 cursor-pointer">
                          ...
                          <span className="text-sm text-slate-700">{tvl} mm</span>
                        </label>
                      );
                    })}
                  </div>
      ...
      {selectingRig && (
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 9999 }}>
          <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-md" onClick={() => setSelectingRig(null)} />
          <div className="bg-white rounded-2xl shadow-[0_0_100px_rgba(0,0,0,0.5)] w-[90vw] max-w-4xl max-h-[85vh] flex flex-col relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between p-4 sm:p-6 border-b border-slate-100 shrink-0">
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="w-2/3 sm:w-3/4 p-4 sm:p-6 overflow-y-auto bg-white">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <button 
                      key={build.id} 
                      onClick={() => { if (selectingRig === 'A') setRigAId(build.id); if (selectingRig === 'B') setRigBId(build.id); setSelectingRig(null); }} 
                    ...
                    {ALL_BUILDS.filter(b => b.brand === selectorBrand && (!selectorGarageOnly || favorites.includes(b.id))).length === 0 && (
                     <div className="col-span-full py-12 text-center text-slate-500">No favorited builds for this brand.</div>
                  )}
                </div>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        <div className="w-full space-y-6">
          <div className="bg-white rounded-2xl shadow-lg overflow-hidden flex flex-col w-full border border-gray-100">
            <div className="w-full h-64 sm:h-72 bg-[#F3F3F3] relative m-0 p-0 overflow-hidden flex items-center justify-center">
              <img src={bike.image} alt={bike.model} className="absolute inset-0 w-full h-full object-contain scale-110" />
            </div>
            <div className="p-8 flex flex-col">
              <div className="flex flex-col items-center pb-6 border-b border-gray-100 gap-1">
                <img src={eMTBData.find(b => b.brand === bike.brand)?.logo} alt={bike.brand} className="h-10 object-contain mb-2" />
                <h2 className="text-lg font-bold text-slate-900 text-center leading-snug">{bike.model} - {build.name} build</h2>
                <p className="text-2xl font-medium text-blue-600">{formatMoney(price)}</p>
              </div>
              <div className="grid grid-cols-2 gap-x-8 gap-y-6 pt-6">
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Material</span><span className="text-sm font-medium text-slate-800">{build.material || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Suspension</span><span className="text-sm font-medium text-slate-800">{bike.suspension}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Fork</span><span className="text-sm font-medium text-slate-800">{build.fork || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Shock</span><span className="text-sm font-medium text-slate-800">{build.shock || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Motor</span><span className="text-sm font-medium text-slate-800">{build.motor || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Battery</span><span className="text-sm font-medium text-slate-800">{build.battery || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Drivetrain</span><span className="text-sm font-medium text-slate-800">{build.drivetrain || 'TBD'}</span></div>
                <div className="flex flex-col gap-1"><span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Brakes</span><span className="text-sm font-medium text-slate-800">{build.brakes || 'TBD'}</span></div>
              </div>
            </div>
          </div>
        </div>

<div className="bg-[#0B1121] rounded-3xl p-8 text-white shadow-xl relative overflow-hidden border border-slate-800">
            {/* Premium Grid & Glow */}
            <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHdpZHRoPSIyMCIgaGVpZ2h0PSIyMCI+PHBhdGggZD0iTTAgMGgyMHYyMEgwVjB6bTEgMWgxOHYxOEgxdjE4eiIgZmlsbD0icmdiYSgyNTUsIDI1NSwgMjU1LCAwLjAzKSIgZmlsbC1ydWxlPSJldmVub2RkIi8+PC9zdmc+')] opacity-50 pointer-events-none"></div>
              ...
              <div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center"><span className="text-slate-400">Rig Price</span><span className="font-semibold">{formatMoney(price)}</span></div>
                    <div className="flex justify-between items-center"><span className="text-slate-400">Estimated Tax</span><span className="font-semibold">{formatMoney(taxAmount || 0)}</span></div>
                    <div className="flex justify-between items-center"><span className="text-slate-400">Down Payment</span><span className="font-semibold text-blue-400">-{formatMoney(Number(downPayment) || 0)}</span></div>
                    <div className="flex justify-between items-center"><span className="text-slate-400">Total Interest</span><span className="font-semibold text-rose-400">+{formatMoney(totalInterest)}</span></div>
                    <div className="flex justify-between items-center"><span className="text-slate-400">Total Financed</span><span className="font-semibold">{formatMoney(totalFinanced || price)}</span></div>
                  </div>
                  <div className="h-px bg-slate-800 w-full"></div>
                ...
                <div className="mt-8 pt-8 border-t border-slate-800">
                  <button onClick={(e) => e.preventDefault()} className="w-full flex items-center justify-center gap-2 bg-slate-800 border border-slate-700 text-slate-400 font-extrabold text-lg py-4 px-6 rounded-xl cursor-not-allowed">Local Dealer Search (Coming Soon)</button>
              ...
              <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-slate-700">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="relative">
                      {/* FIX: Removed the % span and adjusted padding to perfectly match the Down Payment box. Added 8.5% placeholder */}
